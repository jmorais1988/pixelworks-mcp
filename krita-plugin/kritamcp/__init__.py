...
# v2 expansion mixin (document info, layer tree management, pixel PNG bridge,
# filters, selections, transforms, layer export, canvas view, Krita action
# triggers, run_python). Loaded from actions_v2.py when present; the bridge
# falls back to the original command set if the mixin fails to import.
try:
    from .actions_v2 import ActionsV2Part2 as _ActionsV2
    _KritaMCPBases = (_ActionsV2, Extension)
except Exception as _v2_err:
    print("[KritaMCP] actions_v2 unavailable, original command set only:", _v2_err)
    _KritaMCPBases = (Extension,)
...
class KritaMCPExtension(*_KritaMCPBases):
    """Main Krita extension class (v2 actions mixed in when available)."""
    ...
    def execute_command(self, command):
        ...
        try:
            ...
            if action == "new_canvas":
                ...
                handler = getattr(self, "V2_DISPATCH", {}).get(action)
                if handler:
    ...
    def cmd_get_color_at(self, params):
        ...
        if len(pixel_data) >= 4:
            # RGBA (projectionPixelData yields bytes; cast for formatting)
            b, g, r, a = (int(v) for v in pixel_data[0:4])
